...
//	fprintf(stderr,"\n\n   Mensagem = %s\n\n", text);
  putc(text,arq); //Escreve no arquivo de transição;
  fclose(arq);
	if (text=='s')
	{
//		free (text);
	...
	if (text == 'h'){ // requisitando historico
		char historico[1000];
//		char buf[100];
//		le_arq_texto("historico.txt",buf);
//		system("sudo rm historico.txt");
//		system("touch historico.txt");
//		rdwr_arq(buf);
		le_arq_texto("historico.txt",historico);
		send(client_socket, historico, 1000, 0);
